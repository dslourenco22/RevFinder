...
def compare_documents(old_document: ParsedDocument, new_document: ParsedDocument) -> DiffResult:
    ...
    for key in all_keys:
        ...
        if old_item is None and new_item is not None:
            changes.append(RowChange(status="added", match_key=key, new_item=new_item, similarity=0.0))
            continue
        if new_item is None and old_item is not None:
            changes.append(RowChange(status="removed", match_key=key, old_item=old_item, similarity=0.0))
            continue
